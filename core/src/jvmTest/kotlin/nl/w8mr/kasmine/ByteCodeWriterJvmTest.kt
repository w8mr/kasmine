...
class ByteCodeWriterJvmTest {
    ...
    @Test
    fun `write bytes and toByteArray round trip`() {
    ...
    @Test
    fun `ushort with UShort writes big endian`() {
        val w = ByteCodeWriter()
        w.ushort(0x1234u)
        assertArrayEquals(byteArrayOf(0x12, 0x34), w.toByteArray())
    }
    ...
    @Test
    fun `uint writes four bytes big endian`() {
        val w = ByteCodeWriter()
        w.uint(0x12345678u)
    ...
    @Test
    fun `string unary plus decodes hex and writes`() {
        val w = ByteCodeWriter()
        with(w) { +"cafebabe" }
        assertArrayEquals(
    ...
    @Test
    fun `string unary plus with whitespace`() {
        val w = ByteCodeWriter()
        with(w) { +"ca fe ba be" }
        assertArrayEquals(
            byteArrayOf(0xca.toByte(), 0xfe.toByte(), 0xba.toByte(), 0xbe.toByte()),
    ...
    @Test
    fun `instructionOneArgument writes opcode plus two bytes big endian`() {
        val w = ByteCodeWriter()
    ...
    @Test
    fun `instructionTwoArgument writes opcode plus four bytes`() {
        val w = ByteCodeWriter()
    ...
    @Test
    fun `toByteArray returns fresh copy on each call`() {
        val w = ByteCodeWriter()
        w.ubyte(0x01u)
        val a = w.toByteArray()
        w.ubyte(0x02u)

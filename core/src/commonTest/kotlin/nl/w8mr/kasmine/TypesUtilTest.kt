...
class TypesUtilTest {
    ...
    @Test fun `decodeHex converts hex string to bytes`() {
        val bytes = "cafebabe".decodeHex()
        assertContentEquals(byteArrayOf(0xca.toByte(), 0xfe.toByte(), 0xba.toByte(), 0xbe.toByte()), bytes)
    }

    @Test fun `decodeHex strips whitespace`() {
        val bytes = "ca fe ba be".decodeHex()
        assertContentEquals(byteArrayOf(0xca.toByte(), 0xfe.toByte(), 0xba.toByte(), 0xbe.toByte()), bytes)
    }

    @Test fun `decodeHex rejects odd length`() {
        assertFails { "caf".decodeHex() }
    }

    @Test fun `decodeHex empty string returns empty array`() {
        assertContentEquals(byteArrayOf(), "".decodeHex())
    }

    @Test fun `toHex converts bytes to hex string`() {
        val hex = byteArrayOf(0xca.toByte(), 0xfe.toByte(), 0xba.toByte(), 0xbe.toByte()).toHex()
        assertEquals("cafebabe", hex)
    }

    @Test fun `toHex handles single byte`() {
        assertEquals("0a", byteArrayOf(10).toHex())
        assertEquals("ff", byteArrayOf(0xff.toByte()).toHex())
    }

    @Test fun `decodeHex toHex round trip`() {
        val original = "0123456789abcdef"
        val bytes = original.decodeHex()
        assertEquals(original, bytes.toHex())
    }

    @Test fun `decodeHex handles all byte values`() {
    ...
    @Test fun `Opcode objects have correct opcode values`() {
    ...
    @Test fun `Instruction byteSizes are correct`() {
        ...
        assertEquals(3, Instruction.OneArgumentPool(Opcode.GetStatic,
            ConstantPoolType.UTF8String("x")).byteSize)
        assertEquals(5, Instruction.TwoArgumentPool(Opcode.InvokeVirtual,
            ConstantPoolType.UTF8String("x"), ConstantPoolType.UTF8String("y")).byteSize)
    }

    @Test fun `InstructionBlock accumulates byteSize`() {
    ...
    @Test fun `InstructionBlock tracks byteSize with three-byte instruction`() {
        val block = InstructionBlock()
        block.add(Instruction.NoArgument(Opcode.IConst0))

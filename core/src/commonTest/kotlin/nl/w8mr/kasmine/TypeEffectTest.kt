...
class TypeEffectTest {
    ...
    @Test fun `IConstM1 through IConst5 push Integer`() {
        for (op in listOf(Opcode.IConstM1, Opcode.IConst0, Opcode.IConst1, Opcode.IConst2, Opcode.IConst3, Opcode.IConst4, Opcode.IConst5)) {
    ...
    @Test fun `GetField pops Top pushes resolved type`() {
        val fieldRef = ConstantPoolType.FieldRef(
            ConstantPoolType.ClassEntry(ConstantPoolType.UTF8String("Foo")),
            ConstantPoolType.NameAndType(ConstantPoolType.UTF8String("x"), ConstantPoolType.UTF8String("I"))
        )
        val e = Instruction.OneArgumentPool(Opcode.GetField, fieldRef).typeEffect { cp ->
            when (cp) {
                fieldRef -> VerificationType.Integer
                else -> VerificationType.Top
            }
    ...
    @Test fun `PutField pops Top and field type`() {
        val fieldRef = ConstantPoolType.FieldRef(
            ConstantPoolType.ClassEntry(ConstantPoolType.UTF8String("Foo")),
            ConstantPoolType.NameAndType(ConstantPoolType.UTF8String("x"), ConstantPoolType.UTF8String("I"))
        )
        val e = Instruction.OneArgumentPool(Opcode.PutField, fieldRef).typeEffect { cp ->
            when (cp) {
                fieldRef -> VerificationType.Integer
                else -> VerificationType.Top
            }
    ...
    @Test fun `LoadConstant with string pushes Object String`() {
    ...
    @Test fun `LoadConstant with integer pushes Integer`() {
    ...
    @Test fun `New pushes Uninitialized`() {
    ...
    @Test fun `InvokeStatic with void return has no pushes`() {
        val methodRef = ConstantPoolType.MethodRef(
            ConstantPoolType.ClassEntry(ConstantPoolType.UTF8String("Foo")),
            ConstantPoolType.NameAndType(ConstantPoolType.UTF8String("bar"), ConstantPoolType.UTF8String("()V"))
        )
        val e = Instruction.OneArgumentPool(Opcode.InvokeStatic, methodRef).typeEffect { cp ->
            when (cp) {
    ...
    @Test fun `InvokeStatic with return pushes result`() {
        val methodRef = ConstantPoolType.MethodRef(
            ConstantPoolType.ClassEntry(ConstantPoolType.UTF8String("Foo")),
            ConstantPoolType.NameAndType(ConstantPoolType.UTF8String("bar"), ConstantPoolType.UTF8String("()I"))
        )
        val e = Instruction.OneArgumentPool(Opcode.InvokeStatic, methodRef).typeEffect { cp ->
            when (cp) {
    ...
    @Test fun `OneArgumentUShort has no effects`() {
        val e = effect(Instruction.OneArgumentUShort(Opcode.SiPush, 0u))
        assertEquals(emptyList(), e.pops)
        assertEquals(emptyList(), e.pushes)
    }

    @Test fun `TwoArgumentPool has no effects`() {
        val e = effect(Instruction.TwoArgumentPool(Opcode.InvokeVirtual,
            ConstantPoolType.UTF8String("a"), ConstantPoolType.UTF8String("b")))
        assertEquals(emptyList(), e.pops)
        assertEquals(emptyList(), e.pushes)
    }
}
